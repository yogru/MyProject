...
Trex.I.AttachBox = {
	...
	onDeleteAll: function(force) {
		...
		console.log("아아 여기는 attachbox_ui.js의 onDeleteALL 작동하십니까");
		...
		this.datalist.each(function(entry) {
			if (entry.deletedMark == _FALSE) {
				console.log("객체에 함수를 줘서 호출할려고했는데 안됨 개같음");
				if(entry.data.deleteurl!==null){
					$.ajax({
					 url:entry.data.deleteurl,
...
.bind(this), _FALSE);
		...
		$tx.observe(_elDelete, 'click', function() {
			...
			entry.data.callbackDelAfter();
			console.log("단일 파일 삭제  여기  240 line attachbox");
			entry.execRemove();
		}, _FALSE);
